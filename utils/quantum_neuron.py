...
class QuantumNeuron(Neuron, Ket):
    ...
    def feed_forward(self):
        ...
        self.signal = np.tanh(np.dot(self.input_vector, self.qubit.T)) + self.bias
    ...
    def iterate(self):
        ...
        self.qubit -= self.learning_rate * gradient.astype(self.qubit.dtype)
...
if __name__ == "__main__":
    ...
    for x in range(len(sine_wave[0])):
        ket_instance.alpha = sine_wave[0][x]
        ket_instance.beta = sine_wave[1][x]
        ket_instance.iterate()
        if x % 10 == 0:
            print(f"iteration:{x} state:{ket_instance.state} output:{ket_instance.output}")
            print('')

    print(f'prediction: {ket_instance.output} res: {ket_instance.beta}')
